...
`,ut=(t,e)=>t.reduce((t,i)=>(i.defineId?t[i.defineId]=i:i.promise.then(t=>{void 0===e.registry.get(i.name)&&e.registry.define(i.name,t)}),t),{}),_t=t=>({name:t,promise:customElements.whenDefined(t).then(()=>customElements.get(t))}),pt=(t,e,i={},s={})=>{const r=new Event(e,{bubbles:void 0===s.bubbles||s.bubbles,cancelable:Boolean(s.cancelable),composed:void 0===s.composed||s.composed});return r.detail=i,t.dispatchEvent(r),r};class mt extends(lt(at)){static get elementDefinitions(){return ut([_t("ha-checkbox"),_t("ha-formfield"),_t("ha-form-string"),_t("ha-select"),_t("mwc-list-item")],mt)}static get styles(){return ft}static get properties(){return{hass:{},_config:{}}}setConfig(t){this._config={...gt,...t}}get entityOptions(){const t=Object.keys(this.hass.states).filter(t=>["switch","light","group"].includes(t.split(".")[0]));return t.sort(),t}firstUpdated(){this._firstRendered=!0}render(){if(!this.hass||!this._config)return F``;let{header:t}=this._config;if(!t&&this._config.entity){let e=this._config.entity.split(".")[1]||"";e&&(e=e.charAt(0).toUpperCase()+e.slice(1),t=e)}const e=this.entityOptions.map(t=>F`<mwc-list-item value="${t}" ?selected=${t===this._config.entity}>${t}</mwc-list-item>`);return F`
    ...
    `}configChanged(t){if(!this._config||!this.hass||!this._firstRendered)return;const{target:{configValue:e,value:i},detail:{value:s}}=t;this._config=null!=s?{...this._config,[e]:s}:{...this._config,[e]:i},pt(this,"config-changed",{config:this._config})}checkboxConfigChanged(t){if(!this._config||!this.hass||!this._firstRendered)return;const{target:{value:e,checked:i}}=t;this._config={...this._config,[e]:i},pt(this,"config-changed",{config:this._config})}}const bt=254,$t=1e6,vt=[255,255,255],yt=[{name:"Red",hs:[0,100],color:"#ff0000"},{name:"Orange",hs:[30,100],color:"#ff8000"},{name:"Yellow",hs:[60,100],color:"#ffff00"},{name:"Green",hs:[120,100],color:"#00ff00"},{name:"Cyan",hs:[180,100],color:"#00ffff"},{name:"Blue",hs:[240,100],color:"#0000ff"},{name:"Purple",hs:[270,100],color:"#8000ff"},{name:"Pink",hs:[320,100],color:"#ff00aa"}],wt="rgb-light-controller-editor";customElements.define(wt,mt);class xt extends(lt(at)){static get elementDefinitions(){return ut([_t("ha-card"),_t("more-info-light"),_t("ha-switch"),_t("ha-icon"),_t("state-badge"),_t("ha-slider"),_t("ha-hs-color-picker"),_t("ha-select"),_t("mwc-list-item")],xt)}static get properties(){return{hass:{},config:{},_colorPickerValues:{state:!0},_colorMode:{state:!0},_rgbView:{state:!0}}}async firstUpdated(){if(this._firstUpdate=!0,!1!==this.config.color_picker&&this.config.entity.startsWith("light.")&&!customElements.get("ha-hs-color-picker")){try{await Promise.race([customElements.whenDefined("ha-hs-color-picker"),new Promise((t,e)=>setTimeout(()=>e(new Error("timeout")),5e3))])}catch{}this.requestUpdate()}}setConfig(t){if(!t.entity)throw Error("entity required.");this.config={...gt,...t},void 0===this._colorMode&&(this._colorMode=null),void 0===this._rgbView&&(this._rgbView="dots")}static async getConfigElement(){return document.createElement(wt)}static get featureNames(){return{brightness:1,colorTemp:2,effectList:4,color:16,whiteValue:128}}static get cmdToggle(){return{on:"turn_on",off:"turn_off"}}static get entityLength(){return{light:10,switch:1}}getCardSize(){if(!this.config||!this.hass||!this.hass.states[this.config.entity])return 1;let t=0;const e=this.hass.states[this.config.entity];return Array.isArray(e.attributes.entity_id)?e.attributes.entity_id.forEach(e=>t+=this.getEntityLength(e)):t+=this.getEntityLength(e.attributes.entity_id),this.config.group&&(t*=.8),parseInt(t,10)}getEntityLength(t){return/^light\./.test(t)?xt.entityLength.light:/^switch\./.test(t)?xt.entityLength.switch:0}static get styles(){return dt}isEntityOn(t){return"on"===t.state}render(){const t=this.hass.states[this.config.entity];if(!t)return F`
    ...
    `}getEntitiesToShow(t){return t.attributes.entity_id&&Array.isArray(t.attributes.entity_id)?t.attributes.entity_id.map(t=>this.hass.states[t]).filter(Boolean):[t]}createEntityTemplate(t){if(!t||!t.attributes)return F``;const e=this.config.full_width_sliders?"ha-slider-full-width":"",i=this._supportsRgb(t),s=this._supportsWhite(t),r=i&&s;null===this._colorMode&&r?this._colorMode=this._detectColorMode(t):null===this._colorMode&&(this._colorMode=i?"rgb":"white");const o=!r||"rgb"===this._colorMode,n=!!r&&"white"===this._colorMode,c=this._isFixedWhite();return F`
      ${this.createHeader(t)}
    ...
    `}_createRgbViewSwitch(){const t="dots"===this._rgbView;return F`
    ...
    `}_createColorDots(t){const e=t.attributes.hs_color||[0,0],i=this.config.preset_colors||yt;return F`
      <div class="light-entity-card__color-dots">
        ${i.map(i=>{const s=Math.abs(e[0]-i.hs[0])<15&&e[1]>50;return F`
            <button
              class="light-entity-card__color-dot ${s?"light-entity-card__color-dot--selected":""}"
              style="background: ${i.color};"
              title="${i.name}"
              @click=${()=>this._selectPresetColor(i.hs,t)}
            ></button>
          `})}
      </div>
    `}_selectPresetColor(t,e){this.callEntityService({hs_color:t},e)}createHeader(t){if(this.config.hide_header)return F``;const e=this.config.header||t.attributes.friendly_name||t.entity_id;return F`
    ...
    `}_createSaturationSlider(t){if(this.dontShowFeature("color",t)&&!this.config.force_features)return F``;const e=t.attributes.hs_color||[0,0],i=Math.round(e[1]);return F`
      <div class="control light-entity-card-center">
        ...
        ></ha-slider>
        ${this.showPercent(i,0,100)}
      </div>
    `}_setSaturation(t,e){const i=parseInt(t.target.value,10);if(isNaN(i))return;const s=e.attributes.hs_color||[0,0];Math.round(s[1])!==i&&this.callEntityService({hs_color:[s[0],i]},e)}createSpeedSlider(t){return!1===this.config.speed||this.dontShowFeature("speed",t)?F``:F`
      <div class="control light-entity-card-center">
